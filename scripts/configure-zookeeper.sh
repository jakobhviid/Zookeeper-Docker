    ...
    if [[ ${ZOO_AUTHENTICATION} == KERBEROS ]]; then
        ...
        if [[ -z "${ZOO_KERBEROS_PRINCIPAL}" ]]; then
            if [[ -z "${ZOO_KERBEROS_API_URL}" ]]; then
                ...
                response=$(curl --fail --max-time 5 -X POST -H "Content-Type: application/json" -d "{\"username\":\""$ZOO_KERBEROS_API_USERNAME"\", \"password\":\""$ZOO_KERBEROS_API_PASSWORD"\"}" "$ZOO_KERBEROS_API_URL" -o "$keytab_location" && echo "INFO - Using the keytab from the API and a principal name of '"$ZOO_KERBEROS_API_USERNAME"'@'"$ZOO_KERBEROS_REALM"'" || echo "FAIL" )
